...
var g = new WeightedGrid(16, 12);
g.boundaries = walls;

// manhattan distance as heuristic (| x1 - x2 | + | y1 - y2 |)
function heuristic(goal, node){
    var [x1, y1] = goal;
    var [x2, y2] = node;
    return Math.abs(x1 - x2) + Math.abs(y1 -y2);
}

function a_star(grid, start, finish){
    ...
    while (!queue.isEmpty()){
        ...
        for(next of grid.neighbors(current)){
            ...
            if (!(next in cost) || (temp_cost < cost[next])){
                cost[next] = temp_cost;
                var priority = temp_cost + heuristic(finish, next);
...
var result = a_star(g, start, finish);
